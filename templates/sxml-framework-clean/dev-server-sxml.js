...
const server = http.createServer((req, res) => {
  ...
  // 优先查找 dist 目录下的静态资源
  const distPath = path.join(__dirname, 'dist', url.pathname);
  let filePath = fs.existsSync(distPath) ? distPath : ('.' + url.pathname);
...
function handleHtmlRequest(filePath, res, url) {
  try {
    ...
    if (fs.existsSync(sxmlPath)) {
      ...
      // 检查是否是预编译的 HTML（包含 sxml-compiled 标记）
      templateContent = fs.readFileSync(htmlPath, 'utf-8');
      if (templateContent.includes('sxml-compiled') || templateContent.includes('SXML Compiled at')) {
        console.log(`📄 使用预编译 HTML: ${path.basename(htmlPath)}`);
