...
function loadApiTargetByEnv(env) {
  try {
    const suffix = env === 'production' || env === 'prod' ? '.prod' : (env === 'test' ? '.test' : '.dev');
    const filename = `app.config${suffix}.json`;
    ...
    if (fs.existsSync(defaultPath)) {
      const cfg = JSON.parse(fs.readFileSync(defaultPath, 'utf-8'));
      if (cfg && cfg.api && cfg.api.baseUrl) return cfg.api.baseUrl;
    }
  } catch (e) {
