...
class SXMLCompiler {
  ...
  * @param {string} jsonPath - JSON 配置文件路径（可选）
  ...
  compile(sxmlPath, jsPath, outputPath, jsonPath = null) {
    console.log(`📦 开始编译: ${path.basename(sxmlPath)}`);
  ...
  generateScriptTags() {
    ...
    for (const module of deps) {
      ...
      if (module === 'config.js') {
        ...
        scripts.push(`    <script nonce="${this.nonce}">window.APP_CONFIG_URL = '${configFile}';</script>`);
      }
...
<head>
  ...
  <meta http-equiv="Content-Security-Policy" content="default-src 'self'; script-src 'self' 'nonce-${this.nonce}'; style-src 'self' 'nonce-${this.nonce}'; img-src 'self' data: https:; font-src 'self' data:; connect-src ${connectSrc}; frame-ancestors 'none'; base-uri 'self'; form-action 'self';">
...
${this.generateScriptTags()}
  <!-- 页面加载器 -->
    <script nonce="${this.nonce}">window.SXML_PRECOMPILED = true;</script>
  ...
  compileIf(html, data) {
    ...
    const conditionChainRegex = /<(\w+)([^>]*)\s+s:if\s*=\s*["']([^"']+)["']([^>]*)>([\s\S]*?)<\/\1>(?:\s*<(\w+)([^>]*)\s+s:else-if\s*=\s*["']([^"']+)["']([^>]*)>([\s\S]*?)<\/\6>)*(?:\s*<(\w+)([^>]*)\s+s:else\s*([^>]*)>([\s\S]*?)<\/\11>)?/g;
