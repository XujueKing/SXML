...
(function(global) {
  ...
  function Page(options) {
    ...
    global.addEventListener('load', () => {
      ...
      // 确保 SXML Parser 已初始化(延迟一点,等待 sxml.parser.js 的自动解析)
      setTimeout(() => {
        if (!document.body.classList.contains('sxml-ready')) {
          console.warn('⚠️ SXML not parsed automatically, manually triggering...');
          if (window.SXMLParser && !window._sxmlParserInitialized) {
            const parser = new window.SXMLParser(pageInstance);
            parser.parse(document.body);
            document.body.classList.add('sxml-ready');
            window._sxmlParserInitialized = true;
            console.log('✅ SXML Parser manually initialized');
          }
        }
      }, 300);
    });
  ...
  /**
   * 简化版 s:show 处理器 - 用于运行时动态更新
   * 编译时已设置初始状态,这里只需要监听数据变化
   */
  function processSShow() {
    if (!global.currentPage || !global.currentPage.data) {
      console.warn('⚠️  currentPage 或 currentPage.data 不存在');
      return;
    }
    
    const elements = document.querySelectorAll('[s\\:show], [s-show]');
    console.log(`🔍 [s:show] 找到 ${elements.length} 个元素,准备绑定动态更新`);
    
    // 保存原始 setData 方法
    const originalSetData = global.currentPage.setData;
    
    // 重写 setData 以支持 s:show 动态更新
    global.currentPage.setData = function(newData) {
      // 调用原始 setData
      originalSetData.call(this, newData);
      
      // 更新所有 s:show 元素
      elements.forEach(el => {
        const expr = el.getAttribute('s:show') || el.getAttribute('s-show');
        if (!expr) return;
        
        const cleanExpr = expr.replace(/^\s*\{\{([\s\S]+?)\}\}\s*$/, '$1').trim();
        
        try {
          const data = global.currentPage.data;
          ...
          console.error('❌ s:show 更新错误:', cleanExpr, e);
        }
      });
    };
    
    console.log('✅ s:show 动态更新已绑定到 setData');
  }
  ...
  if (document.readyState === 'loading') {
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // DOMContentLoaded 后延迟处理 s:show,确保 Page() 已执行
      setTimeout(processSShow, 200);
    });
  } else {
